...
//    md->library=[md->device newDefaultLibrary];
    NSError*error;
    md->library=[md->device newLibraryWithFile:@"default.metallib" error:&error];
    if (error) puts([[error localizedDescription]UTF8String]);
...
-(id<MTLBuffer>)copyBuffer: (void*)data length:(NSInteger)len {
    return [device newBufferWithBytesNoCopy:data length:[self sz2Page:len] options:MTLResourceStorageModeShared deallocator:nil];
}
...
-(void)runThreads3d: (NSUInteger)width height:(NSUInteger)height depth:(NSUInteger)depth { // for a w x h x d grid
